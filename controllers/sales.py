...
def sales_order_form():
    ...
    if form.process().accepted:        
        ...
        for n in _tmp:
            ...
            _grand_total += _price_cost * _total_qty
...
def validate_sales_order_transaction(form):        
    ...
    if not _id:
        ...
        if (_price.retail_price == 0.0 or _price.wholesale_price == 0.0) and (_id.type_id.mnemonic == 'SAL' or _id.type_id.mnemonic == 'PRO'):
            form.error.item_code = 'Cannot request this item because retail price is zero.'
...
def sales_order_transaction_temporary():    
    ...
    head = THEAD(TR(TH('#'),TH('Item Code'),TH('Item Description'),TH('Category'),TH('UOM'),TH('Quantity'),TH('PCs'),TH('Unit Price'),TH('Excise Tax'),TH('Taxable Value'),TH('Tax %'),TH('Tax Amount'),TH('Total Amount'),TH('Action')))
    ...
    for n in _query:
        ctr += 1        
        _total_amount = generate_total_amount(n.Sales_Order_Transaction_Temporary.quantity, n.Sales_Order_Transaction_Temporary.pieces, n.Item_Master.uom_value, n.Item_Prices.wholesale_price)                    
        ...
        row.append(TR(
            ...
            TD(n.Item_Prices.wholesale_price, _align = 'right'),            
            TD(locale.format('%d',n.Item_Master.selectivetax or 0, grouping = True),_align = 'right'),
            ...
            TD(btn_lnk)))
    body = TBODY(*row)
    foot = TFOOT(TR(TD(H4('TOTAL AMOUNT'), _align = 'right', _colspan = '12'),TD(H4(locale.format('%.2f',grand_total or 0, grouping = True)), _align = 'right'),TD()))
    foot += TFOOT(TR(TD(H4('DISCOUNT'), _align = 'right', _colspan = '12'),TD(H4(INPUT(_class='form-control', _name = 'discount', _id='discount', _value = 0.0)), _align = 'right'),TD()))
    table = TABLE(*[head, body, foot], _class='table table-bordered', _id = 'tblsot')
    return dict(form = form, table = table)
...
def sales_order_report_account_user():
    ...
    for t in db((db.Sales_Order_Transaction.sales_order_no_id == request.args(0)) & (db.Sales_Order_Transaction.delete == False)).select(left = db.Item_Master.on(db.Item_Master.id == db.Sales_Order_Transaction.item_code_id)):
        ctr += 1
        _amount = t.Sales_Order_Transaction.quantity * t.Sales_Order_Transaction.price_cost
        _grand_total += _amount
        if t.Item_Master.uom_value == 1:
            _qty = t.Sales_Order_Transaction.quantity
        else:
            _qty = card(t.Item_Master.id, t.Sales_Order_Transaction.quantity, t.Sales_Order_Transaction.uom)
        _st.append([ctr,t.Item_Master.item_code, t.Item_Master.item_description, t.Sales_Order_Transaction.category_id.mnemonic, t.Sales_Order_Transaction.uom, _qty, locale.format('%.4F',t.Sales_Order_Transaction.price_cost or 0, grouping = True), locale.format('%.2F', _amount or 0, grouping = True)])
