...
def sync_purchase_receipt():
    print 'sync_purchase_receipt', request.args(0)
    _id = db(db.Purchase_Request.id == request.args(0)).select().first()
    _trnx = db((db.Transaction_Prefix.dept_code_id == _id.dept_code_id) & (db.Transaction_Prefix.prefix_key == 'GRV')).select().first()
    _skey = _trnx.current_year_serial_key
    _skey +=1 
    _trnx.update_record(current_year_serial_key = int(_skey), updated_on = request.now, updated_by = auth.user_id)    
    db.Purchase_Receipt.insert(
        purchase_receipt_no_prefix_id = _trnx.id,
        purchase_receipt_no = _skey,
        purchase_receipt_date = request.now,
    ...
    for n in db(db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)).select():
        db.Purchase_Batch_Cost.insert( # Purchase Batch Cost
        ...
        db.Purchase_Receipt_Transaction.insert(
            ...
            remarks = n.item_remarks,
            # old_average_cost = n.,
            # old_landed_cost = n.,
            # difference_quantity = n.,        
...
def purchase_receipt_grid_view():
    ...
    for n in db(db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)).select(orderby = db.Purchase_Request_Transaction.id):        
        ...
        if not _p:
            ...
            _ord = card(_p.quantity_ordered, n.uom)
...
def get_direct_purchase_transaction_id():
    ...
    for n in _query:
        ...
        local_net_amount = float(grand_total) * float(session.exchange_rate_value)        
        ...
        row.append(TR(
            ...
            TD(INPUT(_class='form-control quantity',_type='number',_name='quantity',_value=n.Direct_Purchase_Receipt_Transaction.quantity),_style='width:100px;'),
            TD(INPUT(_class='form-control pieces',_type='number',_name='pieces',_value=n.Direct_Purchase_Receipt_Transaction.pieces),_style='width:100px;'),
    ...
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD('Discount %', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control discount',_type='number', _name = 'discount', _id='discount', _value = 0), _align = 'right'),TD(P(_id='error'))))
        ...
        if request.vars.btnUpdate:
            if isinstance(request.vars.ctr, list):
                ...
                for x in request.vars.ctr:
                    ...
                    db(db.Direct_Purchase_Receipt_Transaction.id == x).update(quantity = request.vars.quantity[row], pieces = request.vars.pieces[row], total_pieces = _qty, total_amount = request.vars.total_amount[row])
                    row+=1
            else:
                print 'not list'
                _qty = int(request.vars.quantity) * int(request.vars.uom) + int(request.vars.pieces)
                db(db.Direct_Purchase_Receipt_Transaction.id == x).update(quantity = request.vars.quantity, pieces = request.vars.pieces, total_pieces = _qty, total_amount = request.vars.total_amount)
...
def purchase_receipt_account_grid_view_validated():
    ...
    # db.Purchase_Request.supplier.writable = False
...
def purchase_receipt_account_validated_transaction(): # .load
    ...
    if db((db.Purchase_Request.id == int(session._id)) & (db.Purchase_Request.status_id == 25)).select().first():
        # print 'request status : ', 25
        response.js = "$('#btnSubmit').attr('disabled','disabled');$('#btnSave').attr('disabled','disabled');$('#btnadd').attr('disabled','disabled');$('#btnDraft').attr('disabled','disabled');$('#btnAbort').attr('disabled','disabled');$('.del').attr('disabled','disabled')"
    ...
    for n in db((db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.delete == False)).select(orderby = db.Purchase_Request_Transaction.id):                        
        ...
        if n.quantity_invoiced == 0:
            ...
            if n.quantity_invoiced > n.quantity_received:
                ...
                _remarks = ''
        # if n.category_id == 2 or n.category_id == 5: # 2 - excess, 5 - short
        #     _remarks = card(n.quantity_received, n.uom)
        ...
        row.append(TR(
            ...
            TD(n.item_code_id.item_code,INPUT(_type='text',  _name='item_code_id', _value = n.item_code_id, _hidden = True)),
            ...
            TD(INPUT(_class='form-control quantity', _type='number',  _name='quantity', _value = _qty), _style='width:100px;'),
            TD(INPUT(_class='form-control pieces', _type='number', _name='pieces', _value = _pcs), _style='width:100px;'),
            TD(INPUT(_class='form-control price_cost',_type='number',_name='price_cost', _style='text-align:right;',_value = locale.format('%.3F',n.price_cost or 0, grouping = True), _readonly=True), _style='width:100px;'),
            TD(INPUT(_class='form-control total_amount', _type='text', _style='text-align:right;', _name='total_amount', _readonly = True, _value = locale.format('%.3F',n.total_amount or 0, grouping = True)),_style="width:100px;"),
            TD(_remarks),
            TD(btn_lnk)))                                           
        _total_net_amount += n.total_amount
        _total_amount = float(_total_net_amount) * int((100 - int(_id.discount_percentage))) / 100    
    _local_amount = float(_id.exchange_rate or 0) * float(_total_amount) 
    _purchase_value = float(_id.landed_cost or 0) * float(_total_amount)    
    body = TBODY(*row)        
    foot  = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),
    TD(INPUT(_id='btnDraft', _name= 'btnDraft', _type='submit', _value='update',_class='btn btn-info')),       
    TD(INPUT(_id='btnValidate', _name='btnValidate', _type= 'submit', _value='validate', _class='btn btn-warning',_disabled=True),TD())))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount ', _id.currency_id.mnemonic, _colspan="2"),TD(INPUT(_class='form-control total_net_amount', _type='text', _id = 'total_net_amount', _style='text-align:right;', _name='total_net_amount', _readonly = True, _value = locale.format('%.3F',_total_net_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Discount', _colspan="2"),TD(INPUT(_class='form-control discount', _type='number', _id = 'discount', _style='text-align:right;', _name='discount', _value = _id.discount_percentage),_style="width:120px;"),TD(),TD()))    
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount    ', _id.currency_id.mnemonic, _colspan="2"),TD(INPUT(_class='form-control foreign_total_amount', _type='text', _id = 'foreign_total_amount', _style='text-align:right;', _name='foreign_total_amount', _readonly = True, _value = locale.format('%.3F',_total_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount (QR)', _colspan="2"),TD(INPUT(_class='form-control local_total_amount', _type='text', _id = 'local_total_amount', _style='text-align:right;', _name='local_total_amount', _readonly = True, _value = locale.format('%.3f',_local_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Purchase Value (QR)', _colspan="2"),TD(INPUT(_class='form-control purchase_value', _type='text', _id = 'purchase_value', _style='text-align:right;', _name='purchase_value', _readonly = True, _value = locale.format('%.3F',_purchase_value or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
            ...
            if isinstance(request.vars['_id'],list):                
                ...
                for x in request.vars['_id']:                                        
                    try:                
                        _quantity = int(request.vars['quantity'][row]) * int(request.vars['uom'][row]) + int(request.vars['pieces'][row])
                        _price_per_piece = float(request.vars['price_cost'][row].replace(',','')) / int(request.vars['uom'][row])         
                        # _row = db((db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.id == int(request.vars._id[row]))).select().first()                        
                        # _row.update_record(quantity = _quantity, total_amount = _price_per_piece * int(_quantity))                            
                        db(db.Purchase_Request_Transaction.id == x).update(quantity_invoiced = _quantity,total_amount = _price_per_piece * int(_quantity))
                    except: 
                        x = 0
                    row += 1                    
            response.flash = 'Transaction updated'
            response.js = "$('#POTtbl').get(0).reload();"     
            # redirect(URL('inventory','account_grid', extension=False), client_side=True)
    elif form.errors:
        response.flash = 'FORM HAS ERROR'    
...
def validated_purchase_receipt(form2):
    ...
    if not _id:
        ...
        _exist = db((db.Purchase_Request_Transaction.item_code_id == _id.id) & (db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.delete == False) & (db.Purchase_Request_Transaction.category_id == request.vars.category_id)).select().first()
        if _exist:            
...
def purchase_receipt_account_view_validate_transaction(): # .load
    ...
    for n in db(db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)).select():
        ...
        _sum_amount += n.total_amount
    
        _net_amount = (_sum_amount * ( 100 - int(n.discount_percentage))) / 100
    _loc_net_amount = float(_net_amount) * float(_pr.exchange_rate)
    body = TBODY(*[row])    
...
def direct_purchase_receipt_transaction_form():
    ...
    for n in _query:
        ...
        dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _type='button  ', _role='button', _class='btn btn-icon-toggle delete', callback=URL(args = n.Direct_Purchase_Receipt_Transaction_Temporary.id, extension = False), **{'_data-id':(n.Direct_Purchase_Receipt_Transaction_Temporary.id)})
        btn_lnk = DIV( dele_lnk)
        row.append(TR(
    ...
    if table.accepts(request,session):
        if request.vars.btnUpdate:
            if isinstance(request.vars.ctr, list):
                ...
                for x in request.vars.ctr:
                    _qty = int(request.vars.quantity[row]) * int(request.vars.uom[row]) + int(request.vars.pieces[row])
                    db(db.Direct_Purchase_Receipt_Transaction_Temporary.id == x).update(quantity = request.vars.quantity[row], pieces = request.vars.pieces[row], total_pieces = _qty, total_amount = request.vars.total_amount[row])
                    row+=1
            else:
                print 'not list'
                _qty = int(request.vars.quantity) * int(request.vars.uom) + int(request.vars.pieces)
                db(db.Direct_Purchase_Receipt_Transaction_Temporary.id == x).update(quantity = request.vars.quantity, pieces = request.vars.pieces, total_pieces = _qty, total_amount = request.vars.total_amount)
...
def save_as_draft():
    # ['landed_cost', 'other_charges','custom_duty_charges','trade_terms_id','exchange_rate','selective_tax','supplier_invoice','supplier_account_code','location_code_id', 'discount']);        
    print 'save_as_draft: ', request.args(0), request.vars.landed_cost, request.vars.other_charges, request.vars.custom_duty_charges,request.vars.trade_terms_id, request.vars.exchange_rate, request.vars.selective_tax,request.vars.supplier_invoice, request.vars.supplier_account_code, request.vars.discount        
    # request.vars.landed_cost, request.vars.other_charges, request.vars.custom_duty_charges,
    # request.vars.trade_terms_id, request.vars.exchange_rate, request.vars.selective_tax, request.vars.supplier_invoice, request.vars.supplier_account_code,
    # request.vars.location_code_id, request.vars.discount
    db(db.Purchase_Request.id == request.args(0)).update(landed_cost = request.vars.landed_cost, other_charges = request.vars.other_charges, custom_duty_charges=request.vars.custom_duty_charges,selective_tax=request.vars.selective_tax,discount_percentage=request.vars.discount)
...
def put_purchase_request_submit_id():   
    db(db.Purchase_Request.id == request.args(0)).update(status_id = 25, submitted = True, received = True, purchase_receipt_date_approved = request.now,purchase_receipt_approved_by = auth.user_id,supplier_account_code=request.vars.supplier_account_code,supplier_account_code_description=request.vars.supplier_account_code_description,trade_terms_id=request.vars.trade_terms_id,supplier_invoice=request.vars.supplier_invoice,currency_id=request.vars.currency_id,remarks=request.vars.remarks)
    session.flash = 'Purchase receipt submitted...'
    redirect(URL('inventory','account_grid'))


def put_purchase_request_save_id():    
    db(db.Purchase_Request.id == request.args(0)).update(supplier_account_code=request.vars.supplier_account_code,supplier_account_code_description=request.vars.supplier_account_code_description,trade_terms_id=request.vars.trade_terms_id,supplier_invoice=request.vars.supplier_invoice,currency_id=request.vars.currency_id,remarks=request.vars.remarks)
...
def purchase_request_transaction_temporary():
    ...
    for n in _query:
        ...
        row.append(TR(
            ...
            TD(btn_lnk)))
    body = TBODY(*row)        
    foot = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount (QR)'), _align = 'right', _colspan='2'),TD(INPUT(_class='form-control local_amount',_type='text', _name = 'local_amount', _id='local_amount',_readonly='true',  _value = local_amount or 0)),TD(_btnUpdate)))    
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount ', _foc.currency_id.mnemonic, _align = 'right', _colspan='2'),TD( INPUT(_class='form-control grand_total',_type='text', _name = 'grand_total', _id='grand_total', _readonly='true',_value = net_amount or 0)),TD()))
    ...
    foot +=  TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount ', _foc.currency_id.mnemonic, _align = 'right', _colspan='2'),TD(INPUT(_class='form-control foreign_amount', _type='text', _name = 'foreign_amount', _id='foreign_amount', _readonly='true', _value = net_amount or 0)),TD()))    
...
def purchase_request_item_code_description_():
...
def purchase_request_transaction_view():
    ...
    if form.process(onvalidation = validateproforma).accepted:
        ...
        response.flash = 'FORM HAS ERRORS'        
    return dict(form = form, _id = _id, _ex = _ex, row = _id)
...
def purchase_request_transaction_view_details():
    ...
    if auth.has_membership(role = 'INVENTORY SALES MANAGER') | auth.has_membership(role = 'INVENTORY'): 
        ...
        head = THEAD(TR(TH('#'),TH('Item Code'),TH('Brand'),TH('Item Description'),TH('UOM'),TH('Category'),TH('Quantity'),TH('Pieces'),TH('MRS Price'),TH('Total Amount'),TH('Action'),_class='bg-primary'))    
    ...
    for n in _query:
        ...
        if auth.has_membership(role = 'INVENTORY SALES MANAGER') | auth.has_membership(role = 'INVENTORY'):
            ...
            row.append(TR(
                ...
                TD(INPUT(_class='form-control price_cost',_name='price_cost',_type='text',_value=locale.format('%.3F',n.Purchase_Request_Transaction.price_cost or 0)), _align = 'right', _style="width:120px;"), 
                TD(INPUT(_class='form-control total_amount',_name='total_amount',_type='text',_value=locale.format('%.3F',n.Purchase_Request_Transaction.total_amount or 0)), _align = 'right', _style="width:120px;"),  
                
                TD(btn_lnk)))
            body = TBODY(*row)        
            foot = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(I('(FX : ',_exc_rate.exchange_rate_value,')')),TD(H4('Net Amount in QR'), _align = 'right', _colspan='2'),TD(H4(INPUT(_class='form-control net_amount_local',_id='net_amount_local',_name='net_amount_local',_type='text',_value=locale.format('%.3F',_local_net_price or 0)), _align = 'right')),TD(_btnUpdate, _style="width:100px;")))            
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Total Amount in ',_id.currency_id.mnemonic ), _align = 'right', _colspan='2'),TD(H4(INPUT(_class='form-control grand_total',_name='grand_total',_type='text',_value=locale.format('%.3F',_total_amount or 0)), _align = 'right')),TD()))
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Discount'), _align = 'right', _colspan='2'),TD(H4(INPUT(_class='form-control discount',_name='discount',_id='discount',_type='number',_value=_id.discount_percentage or 0), _align = 'right')),TD()))
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount in ', _id.currency_id.mnemonic), _align = 'right',_colspan='2'),TD(H4(INPUT(_class='form-control net_amount_foreign',_name='net_amount_foreign',_type='text',_value=locale.format('%.3F',_net_price or 0)), _align = 'right')),TD()))
    table = FORM(TABLE(*[head, body, foot], _class='table', _id = 'tblPRT'))
    if table.accepts(request,session):
        if request.vars.btnRemarks:
            ...
            db(db.Purchase_Request.id == request.args(0)).update(
                ...
                local_currency_value = request.vars.net_amount_local)            
            response.js = "$('#tblPRT').get(0).reload()"
...
def purchase_request_grid():
    ...
    if auth.has_membership(role = 'INVENTORY SALES MANAGER'): # wael approval
        if not _usr:
            ...
            _query = db((db.Purchase_Request.status_id == 19) & (db.Purchase_Request.archives == False) & (db.Purchase_Request.dept_code_id == 3)).select(orderby = ~db.Purchase_Request.id)                     
    elif auth.has_membership(role = 'MANAGEMENT'):  # john approval
    ...
    for n in _query:
        ...
        if auth.has_membership(role = 'MANAGEMENT'): # john approva
            row.append(TR(
                TD(n.purchase_request_date),
                TD(_pr),
                TD(n.dept_code_id.dept_name),
...
def purchase_request_sales_view(): # wael form
    ...
    db.Purchase_Request.proforma_file.writable = False
...
def get_workflow_reports():
    ...
    if int(request.args(0)) == 1:
        ...
        row = []
        head = THEAD(TR(TH('Date'),TH('Purchase Receipt No.'),TH('Department'),TH('Supplier Code'),TH('Status'),TH('Action Required'),TH('Action')))
...
def put_generate_purchase_receipt():
    ...
    _skey += 1
    _tp.update_record(current_year_serial_key = int(_skey), updated_on = request.now, updated_by = auth.user_id)
    db(db.Purchase_Request.id == request.args(0)).update(purchase_receipt_no_prefix_id = _tp.id, purchase_receipt_no = _skey,purchase_receipt_approved_by = auth.user_id, purchase_receipt_date_approved = request.now, status_id = 18)    
...
def purchase_receipt_warehouse_grid_process():
    ...
    _id = db((db.Purchase_Request_Transaction.selected == True) & (db.Purchase_Request_Transaction.consolidated == False)).select().first()
    ...
    for n in db((db.Purchase_Request_Transaction.selected == True) & (db.Purchase_Request_Transaction.consolidated == False) & (db.Purchase_Request_Transaction.updated_by == auth.user_id)).select(
        db.Purchase_Request_Transaction.item_code_id, 
        db.Purchase_Request_Transaction.category_id,
        _qty,orderby = ~db.Purchase_Request_Transaction.item_code_id, groupby = db.Purchase_Request_Transaction.item_code_id | db.Purchase_Request_Transaction.category_id):

        ctr += 1              
        _cls = db(db.Purchase_Request_Transaction.item_code_id == n.Purchase_Request_Transaction.item_code_id).select().first()
        
        cut_lnk = A(I(_class='fas fa-cut'), _title='Cut Row', _type='button  ', _role='button', _class='btn btn-icon-toggle cut',callback=URL( args = [_cls.item_code_id,_cls.purchase_request_no_id]), **{'_data-id':(_cls.item_code_id), '_data-pr':(_cls.purchase_request_no_id)})
        btn_lnk = DIV(cut_lnk)
        _itm = db(db.Item_Master.id == n.Purchase_Request_Transaction.item_code_id).select().first()

        row.append(TR(
            TD(ctr),
            TD(n.Purchase_Request_Transaction.item_code_id.item_code, INPUT(_type='text',_name='item_code_id', _hidden = True, _value= n.Purchase_Request_Transaction.item_code_id)),
            TD(_itm.item_description.upper(),INPUT(_type='text',_name='qty', _hidden = True, _value= n[_qty])),
            TD(_itm.uom_value,INPUT(_type='number',_name='uom', _hidden = True, _value= _itm.uom_value)),
            TD(n.Purchase_Request_Transaction.category_id.description,INPUT(_type='number',_name='category_id', _hidden = True, _value= n.Purchase_Request_Transaction.category_id)),
            TD(cut_lnk))) 
    session.ctr = ctr  
    ...
    if form.accepts(request, session):        
        ...
        _skey += 1
        _tp.update_record(current_year_serial_key = int(_skey), updated_on = request.now, updated_by = auth.user_id)
        db(db.Purchase_Request.id == _id.purchase_request_no_id).update(purchase_receipt_no_prefix_id = _tp.id, purchase_receipt_no = _skey,purchase_receipt_approved_by = auth.user_id, 
            purchase_receipt_date_approved = request.now, status_id = 17, supplier_code_id = _supplier.id,location_code_id = form.vars.location_code_id)
        # db.Purchase_Receipt_Warehouse_Consolidated.insert(
        #     purchase_receipt_no_prefix_id = _tp.id, 
        #     purchase_receipt_no = _skey,
        #     purchase_receipt_approved_by = auth.user_id, 
        ...
        # _proc = db((db.Purchase_Request_Transaction.selected == True) & (db.Purchase_Request_Transaction.consolidated == False)).select(db.Purchase_Request_Transaction.purchase_order_no_id, groupby = db.Purchase_Request_Transaction.purchase_order_no_id)
        # _proc = db((db.Purchase_Request_Transaction.selected == True) & (db.Purchase_Request_Transaction.consolidated == False) & (db.Purchase_Request_Transaction.updated_by == auth.user_id)).select(db.Purchase_Request_Transaction.ALL)
        # for p in _proc:                                    
        #     _pr = db(db.Purchase_Receipt_Warehouse_Consolidated.purchase_receipt_no == int(_skey)).select().first()
        ...
        # for x in db(db.Purchase_Request_Transaction.purchase_order_no_id == int(p.purchase_order_no_id)).select():
        #     x.update_record(consolidated = True)        
        ...
        #         _prt = db((db.Purchase_Request_Transaction.item_code_id == _item_code_id) & (db.Purchase_Request_Transaction.purchase_order_no_id == int(_id.purchase_order_no_id))).select().first()                  
        #         db.Purchase_Receipt_Transaction_Consolidated.insert(                    
        #             purchase_receipt_no_id = int(_pr.id),
        ...
        #         _prt = db((db.Purchase_Request_Transaction.item_code_id == int(request.vars['item_code_id'][x])) & (db.Purchase_Request_Transaction.purchase_order_no_id == int(_id.purchase_order_no_id))).select().first()  
...
def purchase_receipt_warehouse_grid(): # hakim's form
    ...
    for n in db((db.Purchase_Request.status_id == 17) & (db.Purchase_Request.consolidated == False)).select(orderby = db.Purchase_Request.id):
        view_lnk = A(I(_class='fas fa-search'), _title='View Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('procurement','purchase_receipt_warehouse_grid_view', args = n.id, extension = False))        
        insu_lnk = A(I(_class='fas fa-car-crash'), _title='Insurance', _type='button ', _role='button', _class='btn btn-icon-toggle', _disabled = True)
        purh_lnk = A(I(_class='fas fa-shopping-bag'), _title='Generage Purchase Order', _type='button ', _role='button', _class='btn btn-icon-toggle', _disabled = True)    
        clea_lnk = A(I(_class='fas fa-archive'), _title='Clear Row', _type='button ', _role='button', _class='btn btn-icon-toggle', _disabled = True)
...
def purchase_receipt_warehouse_grid_consolidated():
    row = []
    head = THEAD(TR(TH('Date'),TH('Purchase Receipt No.'),TH('Purcase Order No.'),TH('Purchase Request No.'),TH('Department'),TH('Supplier Code'),TH('Location'),TH('Status'),TH('Action Required'),TH('Action'),_class='bg-success'))
    for n in db(db.Purchase_Request.status_id == 18).select(orderby = db.Purchase_Request.id):
        view_lnk = A(I(_class='fas fa-search'), _title='View Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('procurement','purchase_receipt_warehouse_grid_view', args = n.id, extension = False))
        edit_lnk = A(I(_class='fas fa-user-edit'), _title='Edit Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('procurement','purchase_receipt_warehouse_grid_consolidated_processed', args = n.id, extension = False))
        dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _type='button  ', _role='button', _class='btn btn-icon-toggle disabled')
...
def purchase_receipt_warehouse_grid_consolidated_processed():
    ...
    for n in db(db.Purchase_Request.id == request.args(0)).select():        
        ...
        # _loc_amount += _po.total_amount_after_discount * _foc.exchange_rate_value
        # _pr = db(n.purchase_order_no_id == db.Purchase_Order.id).select().first()
        # _consl = db(db.Purchase_Receipt_Warehouse_Consolidated.id == n.purchase_request_no_id).select().first()
    ...
    thead = THEAD(TR(
        TD(DIV(LABEL('Location:'),DIV(_id.location_code_id.location_name),_class='form-group'),_colspan='3'),TD(),
        TD(DIV(LABEL('Supplier:'),DIV(_id.supplier_code_id.supp_name),_class='form-group'),_colspan='2'),TD(),TD(),TD(),TD()))        
    ...
    for m in db((db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated_New_Item.partial == False) & (db.Purchase_Receipt_Transaction_Consolidated_New_Item.delete == False)).select():        
        ...
        if m.uom == 1:
            _mpcs = INPUT(_type='number', _class='form-control pieces', _value = 0, _disabled = True), INPUT(_type='number', _class='form-control pieces',_name='pieces',_value = 0, _hidden = True)           
        else:
            _mpcs = INPUT(_type='number', _class='form-control pieces',_name='pieces',_value = _mpcs)      
        ...
        if m.new_item == True:
            trow.append(TR(
                ...
                TD(m.category_id.description),
                TD(INPUT(_type='text', _class='form-control date',_name='production_date', _value = m.production_date), _style="width:120px;"),
                TD(INPUT(_type='text', _class='form-control date',_name='expiration_date', _value = m.expiration_date), _style="width:120px;"),
                TD(INPUT(_type='number', _class='form-control quantity',_name='quantity', _value= _mqty, _align = 'right'), _style="width:120px;"),
        ...
        if isinstance(request.vars['_id'], list):
            ...
            for x in request.vars['_id']:
                try:                                
                    ...
                    _prc = db((db.Purchase_Request_Transaction.purchase_request_no_id == request.args(0)) & (db.Purchase_Request_Transaction.item_code_id == _id.id)).select().first()
                    ...
                    if _prc:                    
                        ...
                        _prc.update_record(quantity_received = _qty, production_date = request.vars['production_date'][row], expiration_date = request.vars['expiration_date'][row], total_amount = _total_amount)                                    
                        # print '_prc', _id.id, request.vars['item_code'][row]
                    else:               
                        # print 'no prc'                                     
                        _qty = int(request.vars['quantity'][row]) * int(request.vars['uom'][row]) + int(request.vars['pieces'][row])
                        # print 'new: ', _qty                        
                        _npc.update_record(quantity_received = _qty, production_date = request.vars['production_date'][row], expiration_date = request.vars['expiration_date'][row])
                except:
                    _n =0
                row += 1
        else:                       
