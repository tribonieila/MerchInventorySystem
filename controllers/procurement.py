...
def purchase_receipt_grid():
    ...
    for n in db(db.Purchase_Receipt.status_id == 18).select():
        ...
        btn_lnk = DIV(view_lnk, appr_lnk, reje_lnk, prin_lnk, clea_lnk)
...
def puchase_receipt_account_grid(): # manoj
    ...
    for n in db(((db.Purchase_Receipt_Warehouse_Consolidated.status_id == 18)| (db.Purchase_Receipt_Warehouse_Consolidated.status_id == 21)) & (db.Purchase_Receipt_Warehouse_Consolidated.draft == False)).select(db.Purchase_Receipt_Warehouse_Consolidated.ALL , orderby = ~db.Purchase_Receipt_Warehouse_Consolidated.id):
        # _prnt = db(db.Purchase_Receipt.purchase_receipt_no_id_consolidated == n.id).select().first()
        if n.status_id == 21:
            ...
            prin_lnk = A(I(_class='fas fa-print'), _title='Print', _type='button ', _role='button', _class='btn btn-icon-toggle', _target = '_blank', _href = URL('procurement','purchase_receipt_reports', args = n.id, extension = False))            
        else:        
            prin_lnk = A(I(_class='fas fa-print'), _title='Print', _type='button ', _role='button', _class='btn btn-icon-toggle', _disabled = True)            
...
def purchase_receipt_account_grid_view_transaction(): # .load
    ...
    trow=[]     
    head = THEAD(TR(TH('#'),TH('Item Code'),TH('Item Description'),TH('UOM'),TH('Category'),TH('Purchase Qty'),TH('Warehouse Receipt Qty'),TH('Unit Price'),TH('Total Amount'),TH('Remarks'),TH('Action'),_class='bg-success'))        
...
def purchase_receipt_account_grid_view_validate():
    ...
    #     redirect(URL('procurement','purchase_receipt_account_view_validate_transaction', args = request.args(0)))
        ...
        form3 = SQLFORM.factory(
            ...
            Field('exchange_rate','decimal(15,6)', default = _curr.exchange_rate_value, widget=SQLFORM.widgets.double.widget),
            ...
            Field('other_charges','decimal(15,6)', default = 0.0,  widget=SQLFORM.widgets.double.widget),    
            Field('custom_duty_charges','decimal(15,6)', default = 0.0, widget=SQLFORM.widgets.double.widget),            
            Field('selective_tax','decimal(15,6)', default = 0.0, widget=SQLFORM.widgets.double.widget),
        ...
        return dict(table = table, form3 = form3)

def landed_cost_validation():
    if request.vars.landed_cost < request.vars.exchange_rate:
        response.js = "jQuery(landed_cost_alert())"
    else:
        response.js = "$('#btnProceed').removeAttr('disabled');"
...
def validate_account_transaction():
    ...
    for x in request.vars['item_code_id']:
        ...
        if row < 1:
            response.js = "jQuery(compute(%s, %s, %s, %s, %s))" % (request.vars['item_code_id'], request.vars['quantity'], request.vars['uom'], request.vars['pieces'],request.vars['price_cost'])
        else:
            response.js = "jQuery(computed(%s, %s, %s, %s, %s))" % (request.vars['item_code_id'], request.vars['quantity'], request.vars['uom'], request.vars['pieces'],request.vars['price_cost'])

        # print request.now.time(), 'row: ', row,  _total_amount, _grand_total
        row += 1
...
def purchase_receipt_account_validate_transaction(): # .load
    ...
    head = THEAD(TR(TH('#'),TH('Item Code'),TH('Item Description'),TH('UOM'),TH('Category'),TH('Ordered Qty'),TH('Warehouse Receipt Qty'),TH('Invoice Qty'),TH('Invoice Pcs'),TH('Unit Price'),TH('Total Amount'),TH('Remarks'),TH('Action'),_class='bg-success'))        
    for n in db(db.Purchase_Receipt_Transaction_Consolidated.purchase_receipt_no_id == request.args(0)).select(db.Item_Master.ALL, db.Purchase_Receipt_Transaction_Consolidated.ALL, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Receipt_Transaction_Consolidated.item_code_id)):
        ctr += 1
        dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _type='button  ', _role='button', _class='btn btn-icon-toggle del', _disabled = True, callback = URL(args = n.Purchase_Receipt_Transaction_Consolidated.id, extension = False), **{'_data-ct':(n.Purchase_Receipt_Transaction_Consolidated.id)})
        btn_lnk = DIV(dele_lnk)                
        ...
        _qty = INPUT(_type='number', _class='form-control quantity', _id = 'quantity', _name='quantity', _value= _qty, _onchange="ajax('/procurement/validate_account_transaction',['item_code_id', 'quantity', 'pieces', 'uom', 'price_cost']); ")
        # _qty = INPUT(_type='number', _class='form-control quantity', _id = 'quantity', _name='quantity', _value = _qty, _onchange="jQuery((%s))"))
        if n.Purchase_Receipt_Transaction_Consolidated.uom == 1:
            _pcs = INPUT(_type='number', _class='form-control', _value = 0, _disabled = True), INPUT(_type='number', _class='form-control', _id = 'pieces', _name='pieces',_value = 0, _hidden = True)            
        else:
            _pcs = INPUT(_type='number', _class='form-control pieces', _id = 'pieces', _name='pieces',_value = _pcs , _onchange="ajax('/procurement/validate_account_transaction',['item_code_id', 'quantity', 'pieces', 'uom', 'price_cost']);")
        ...
        row.append(TR(
            ...
            TD(INPUT(_class='form-control price_cost', _type='number', _id = 'price_cost', _style="text-align:right;", _name='price_cost', _value= locale.format('%.3F',_pot.price_cost or 0, grouping = True), _onchange = "ajax('/procurement/validate_account_transaction',['item_code_id', 'quantity', 'pieces', 'uom', 'price_cost']);"),  _style="width:120px;"),
            TD(INPUT(_class='form-control', _type='text', _id = 'total_amount', _style='text-align:right;', _name='total_amount', _readonly = True, _value = locale.format('%.3F',_total_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD(btn_lnk)))        
        ...
        #         dele_lnk = A(I(_class='fas fa-trash-alt'), _title='Delete Row', _type='button  ', _role='button', _class='btn btn-icon-toggle delete',  _disabled = True, callback = URL(args = x.id, extension = False), **{'_data-id':(x.id)})          
        #         btn_lnk = DIV(dele_lnk)
        ...
        #             TD(n.Item_Master.item_description),
        #             TD(n.Purchase_Receipt_Transaction_Consolidated.uom, INPUT(_type='numbers', _hidden = True, _value= x.uom)),
    ...
    for y in db((db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated_New_Item.new_item == False) ).select():
        ...
        if y.uom == 1:
            ...
            _new_pcs = INPUT(_type='number', _class='form-control pieces', _id = 'pieces', _name='pieces', _value = y.pieces, _onchange="ajax('/procurement/validate_account_transaction',['item_code_id', 'quantity', 'pieces', 'uom', 'price_cost']);")
        if y.category_id == 1:
            _remarks = 'Damaged entry'
        else:
            _remarks = ''        
        _price_cost = db(db.Purchase_Order_Transaction.item_code_id == y.item_code_id).select().first()
        _cprice_cost = float(y.price_cost) / int(y.uom)
        _qty = y.quantity / y.uom
        
        _new_qty = INPUT(_type='number', _class='form-control', _id = 'quantity', _name='quantity', _value= _qty, _onchange="ajax('/procurement/validate_account_transaction',['item_code_id', 'quantity', 'pieces', 'uom', 'price_cost']); ")
        
        _total_amount_3 = float(y.price_cost) * int(_qty)
        total_amount_3 += _total_amount_3
        
        row.append(TR(
            TD(ctr),
            TD(y.item_code_id.item_code,INPUT(_type='numbers', _id='item_code_id', _name='item_code_id', _hidden = True, _value = y.item_code_id) ),
            TD(y.item_description,INPUT(_type='text', _id='production_date', _name='production_date', _hidden = True, _value = n.Purchase_Receipt_Transaction_Consolidated.production_date),INPUT(_type='text', _id='expiration_date', _name='expiration_date', _hidden = True, _value = n.Purchase_Receipt_Transaction_Consolidated.expiration_date)),
            TD(y.uom,INPUT(_type='text', _id = 'uom', _name='uom', _hidden = True, _value= y.uom)),
            TD(y.category_id.mnemonic,INPUT(_type='numbers', _id='category_id', _name='category_id', _hidden = True, _value = y.category_id)),
            TD(card(0,y.uom),INPUT(_type='number', _id='uom', _name='uom', _hidden = True, _value = y.uom)),
            TD(card(y.total_pieces, y.uom),INPUT(_type='numbers', _id = '_cquantity', _name='_cquantity',  _hidden = True, _value= y.total_pieces)),
            TD(_new_qty, INPUT(_type='number', _id='flanded_cost', _name='flanded_cost',_hidden = True, _value = n.Purchase_Receipt_Transaction_Consolidated.price_cost),_align = 'right', _style="width:120px;"),
            ...
            TD(INPUT(_class='form-control price_cost', _type='number', _id = 'price_cost',   _style="text-align:right;", _name='price_cost', _value= locale.format('%.3F',y.price_cost or 0, grouping = True), _onchange="ajax('/procurement/validate_account_transaction',['item_code_id', 'quantity', 'pieces', 'uom', 'price_cost']);"),  _align = 'right', _style="width:120px;"),
    ...
    _total_net_amount = float(_total_amount_1) + float(total_amount_3) 
    _total_amount = float(_total_net_amount) * int((100 - n.Purchase_Receipt_Transaction_Consolidated.discount_percentage)) / 100    
    _cur = db(db.Currency_Exchange.id == _po.currency_id).select().first()
    _local_amount = float(_total_amount) * float(_cur.exchange_rate_value)
    ...
    TD(INPUT(_id='btnSubmit', _type='submit', _value='submit',_class='btn btn-success')),    
    TD(INPUT(_id='btnAbort', _type='button', _value='abort', _class='btn btn-danger')),TD()))
    ...
    # TD(INPUT(_class='form-control', _type='numbers', _id = 'total_amount', _style='text-align:right;', _name='total_amount', _readonly = True, _value = locale.format('%.6F',_total_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD(btn_lnk)))        
    # foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount'),TD(_po.currency_id.mnemonic,' ', locale.format('%.6F',_total_net_amount or 0, grouping = True),_align = 'right'),TD(),TD()))
    # foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Discount'),TD(locale.format('%.6F', n.Purchase_Receipt_Transaction_Consolidated.discount_percentage or 0, grouping = True), _align = 'right'),TD(),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount ', _po.currency_id.mnemonic),TD(INPUT(_class='form-control', _type='text', _id = 'total_net_amount', _style='text-align:right;', _name='total_net_amount', _readonly = True, _value = locale.format('%.3F',_total_net_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Discount'),TD(INPUT(_class='form-control', _type='number', _id = 'discount', _style='text-align:right;', _name='discount', _value = locale.format('%.3F',n.Purchase_Receipt_Transaction_Consolidated.discount_percentage or 0, grouping = True)),_style="width:120px;"),TD(),TD()))    
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount    ', _po.currency_id.mnemonic),TD(INPUT(_class='form-control', _type='text', _id = 'foreign_total_amount', _style='text-align:right;', _name='foreign_total_amount', _readonly = True, _value = locale.format('%.3F',_total_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount (QR)'),TD(INPUT(_class='form-control', _type='text', _id = 'local_total_amount', _style='text-align:right;', _name='local_total_amount', _readonly = True, _value = locale.format('%.3F',_local_amount or 0, grouping = True)),_style="width:120px;"),TD(),TD()))
    ...
    if form.process().accepted:    
        ...
        for x in xrange(ctr):  
            if int(ctr) <= 1:           
                # print 'one: ', x, ctr, request.vars['item_code_id'], request.vars['quantity']
                ...
                # if int(request.vars['category_id'][x]) == 1 and int(request.vars['item_code_id']):
                # print 'damages', y.id            
                if int(request.vars['_cquantity']) > int(_total_pcs): # updated or insert to purchase receipt transaction if excess
                    ...
                    db.Purchase_Receipt_Transaction.update_or_insert(
                        ...
                        _prtc = db((db.Purchase_Receipt_Transaction_Consolidated.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated.item_code_id == int(request.vars['item_code_id']))).select().last()
                        if _prtc:
                            ...
                            _prtcn = db((db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated_New_Item.item_code_id == int(request.vars['item_code_id']))).select().last()
                            _diff = int(request.vars['_cquantity']) - int(_total_pcs)
                    ...
                    if not _stk_fil:
                        ...
                        #     (db.Stock_File.item_code_id == request.vars['item_code_id']) & (db.Stock_File.location_code_id == session.location_code_id),
                        #     item_code_id = request.vars['item_code_id'],
                        #     location_code_id = session.location_code_id,
                ...
                print 'many: ', x, ctr, request.vars['item_code_id'][x], request.vars['quantity'][x], request.vars['_cquantity'][x]
                ...
                # print 'damages', y.id            
                if int(request.vars['_cquantity'][x]) > int(_total_pcs): # updated or insert to purchase receipt transaction if excess
                    ...
                    if _dpr:
                        _total = int('{:,d}'.format(abs(_total_pcs))) * float(request.vars['price_cost'][x].replace(',',''))
                        db.Direct_Purchase_Receipt_Transaction.insert(
                            purchase_receipt_no_id = session._dpr,
                        ...
                        db.Direct_Purchase_Receipt_Transaction.insert(
                            ...
                            price_cost = float(request.vars['price_cost'][x].replace(',','')),
                            excessed = True)                              
                elif int(request.vars['_cquantity'][x]) != int(_total_pcs):
                    db.Purchase_Receipt_Transaction.update_or_insert(
                        purchase_receipt_no_id_consolidated = request.args(0),
                        purchase_receipt_no_id = _pr.id,
                        item_code_id = request.vars['item_code_id'][x],
                ...
                elif int(request.vars['_cquantity'][x]) < int(_total_pcs): # updated or insert to purchase receipt transaction if short                            
                        ...
                        _prwc.update_record(status_id = 17, draft = True)
                        # _prowc = db(db.Purchase_Receipt_Ordered_Warehouse_Consolidated.purchase_receipt_no_id == request.args(0)).select().first()
                        _prtc = db((db.Purchase_Receipt_Transaction_Consolidated.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated.item_code_id == int(request.vars['item_code_id'][x]))).select().last()
                        if _prtc:
                            ...
                            _prtcn = db((db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated_New_Item.item_code_id == int(request.vars['item_code_id'][x]))).select().last()
                            _diff = int(request.vars['_cquantity'][x]) - int(_total_pcs)
                            _prtcn.update_record(
                                difference_quantity = _diff, 
                                selected = False, 
                ...
                elif int(request.vars['_cquantity'][x]) == int(_total_pcs): # updated or insert to purchase receipt transaction  if normal                          
                    db.Purchase_Receipt_Transaction.update_or_insert(
                        purchase_receipt_no_id_consolidated = request.args(0),
                        purchase_receipt_no_id = _pr.id,
                        item_code_id = request.vars['item_code_id'][x],
                        ...
                        received = True)    
                    _batch_quantity = int(request.vars['quantity'][x]) * int(request.vars['uom'][x]) + int(request.vars['pieces'][x])
                    db.Purchase_Batch_Cost.insert(
                        item_code_id = request.vars['item_code_id'][x],
                        purchase_receipt_date = request.now,
                        batch_cost = float(request.vars['price_cost'][x].replace(',','')),
                        supplier_price = float(request.vars['flanded_cost'][x]),
                        batch_quantity = _batch_quantity,
                        batch_production_date = str(request.vars['production_date'][x]),
                        batch_expiry_date = str(request.vars['expiration_date'][x]))
                    if not _stk_fil:
                        db.Stock_File.insert(
                            item_code_id = request.vars['item_code_id'][x],
                            location_code_id = session.location_code_id,
                            opening_stock = int(request.vars['quantity'][x]),
                            closing_stock = int(request.vars['quantity'][x]),
                            last_transfer_qty = int(request.vars['quantity'][x]))
                        # _ave_cst = (int(request.vars['quantity'][x]) * float(_itm_prc.average_cost)) + (float(_itm_prc.most_recent_landed_cost) * int(request.vars['quantity'][x])) / (int(_stk_fil.opening_stock) + int(request.vars['quantity'][x]))

                    else:
                        _opn_stk = int(_stk_fil.opening_stock) + int(request.vars['quantity'][x])
                        _clo_stk = int(_stk_fil.closing_stock) + int(request.vars['quantity'][x])
                        _stk_fil.update_record(
                            opening_stock = _opn_stk,
                            closing_stock = _clo_stk,
                            last_transfer_qty = int(request.vars['quantity'][x]))
                        ...
                        # db.Stock_File.update_or_insert(
                        #     (db.Stock_File.item_code_id == request.vars['item_code_id'][x]) & (db.Stock_File.location_code_id == session.location_code_id),
                        #     item_code_id = request.vars['item_code_id'][x],
                        #     location_code_id = session.location_code_id,
                        #     closing_stock = _tot_stk,
                        #     last_transfer_qty = int(request.vars['quantity'][x])
                        # )
                elif int(request.vars['category_id'][x]) == 1: # updated or insert to purchase receipt transaction  if damaged
                    # print 'damages goes here'
                    db.Purchase_Receipt_Transaction.update_or_insert(
                        purchase_receipt_no_id_consolidated = request.args(0),
                        purchase_receipt_no_id = _pr.id,
    ...
    if form2.process(onvalidation = validate_purchase_receipt).accepted:
        ...
        response.flash = 'RECORD SAVE'
        response.js = '$("#PRTCNItbl").get(0).reload();'
    elif form2.errors:
        response.flash = 'FORM HAS ERROR'        
...
def purchase_receipt_account_view_validate_transaction(): # .load
    ...
    ctr = _total_amount = 0
    row = []
    for n in db((db.Purchase_Receipt_Transaction.purchase_receipt_no_id_consolidated == request.args(0)) & (db.Purchase_Receipt_Transaction.excessed == False)).select(orderby = db.Purchase_Receipt_Transaction.id):
        ctr += 1
        if int(n.difference_quantity) > 0:
            _remarks = n.remarks , ' ', card(n.difference_quantity, n.uom)
        else:
            _remarks = n.remarks
    ...
    _net_amount = (_total_amount * ( 100 - _pr.discount_percentage)) / 100
    ...
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount:'),TD(_pr.currency_id.mnemonic,' ', locale.format('%.3F',_total_amount or 0, grouping = True), _align = 'right'),TD(),TD()))
...
def validate_purchase_receipt(form2):
    ...
    if not _id:
        ...
        elif db((db.Purchase_Receipt_Transaction_Consolidated_New_Item.item_code_id == _id.id) & (db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0))).select().first():
            form2.errors.item_code = 'Item code ' + str(request.vars.item_code) + ' already exist.'
        else:    
...
def purchase_request_transaction_temporary():
    ...
    body = TBODY(*row)        
    foot = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount (QR)'), _align = 'right', _colspan='2'),TD(INPUT(_class='form-control',_type='text', _name = 'local_amount', _id='local_amount', _disabled = True, _value = locale.format('%.2F',local_amount or 0, grouping = True))),TD()))    
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD('Total Amount', _align = 'right', _colspan='2'),TD(_foc.currency_id.mnemonic, INPUT(_class='form-control',_type='text', _name = 'net_amount', _id='net_amount', _disabled = True , _value = locale.format('%.2F',net_amount or 0, grouping = True))),TD()))
    foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD('Discount %', _align = 'right', _colspan='2'),TD(INPUT(_class='form-control',_type='number', _name = 'discount', _id='discount', _value = 0), _align = 'right'),TD(P(_id='error'))))
    foot +=  TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD('Net Amount', _align = 'right', _colspan='2'),TD(_foc.currency_id.mnemonic, INPUT(_class='form-control', _type='text', _name = 'foreign_amount', _id='foreign_amount', _disabled = True,  _value = locale.format('%.2F',net_amount or 0, grouping = True))),TD()))    
...
def purchase_request_item_code_description():
    ...
    # print 'dept: ', session.dept_code_id, session.supplier_code_id, request.vars.item_code
    if not _icode:
        # response.js = "$('#btnadd').attr('disabled','disabled')"
    ...
    for n in _query:
        ...
        if auth.has_membership(role = 'INVENTORY SALES MANAGER') | auth.has_membership(role = 'INVENTORY'):
            row.append(TR(
                ...
                TD(btn_lnk)))
            body = TBODY(*row)        
            foot =  TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount (QR)'), _align = 'right'),TD(H4(locale.format('%.2F',_foc_amount or 0, grouping = True), _align = 'right')),TD()))
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Total Amount '), _align = 'right'), TD(H4(_id.currency_id.mnemonic, ' ' ,locale.format('%.2F',grand_total or 0, grouping = True), _align = 'right')),TD()))
            ...
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount'), _align = 'right'),TD(H4(_id.currency_id.mnemonic, ' ' ,locale.format('%.2F',_id.total_amount_after_discount or 0, grouping = True), _align = 'right')),TD()))            
        else:
            row.append(TR(
                TD(ctr),
                TD(n.Purchase_Request_Transaction.item_code_id.item_code),
                ...
                TD(btn_lnk)))
            body = TBODY(*row)        
            foot = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount (QR)'), _align = 'right'),TD(H4(locale.format('%.2F',_foc_amount or 0, grouping = True), _align = 'right')),TD()))            
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Total Amount '), _align = 'right'),TD(H4(_id.currency_id.mnemonic,' ',locale.format('%.2F',grand_total or 0, grouping = True), _align = 'right')),TD()))
            ...
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('Net Amount '), _align = 'right'),TD(H4(_id.currency_id.mnemonic,' ',locale.format('%.2F',_id.total_amount_after_discount or 0, grouping = True), _align = 'right')),TD()))
...
def document_register_grid_processed():
    ...
    for n in db(db.Document_Register).select():
...
def document_register_grid_process():
    ...
    form = SQLFORM.factory(
        Field('supplier_code_id', 'reference Supplier_Master',ondelete = 'NO ACTION', label = 'Supplier Code', requires = IS_IN_DB(db, db.Supplier_Master.id,'%(supp_code)s - %(supp_name)s', zero = 'Choose Supplier Code')),
        Field('location_code_id','reference Location', ondelete = 'NO ACTION', label = 'Location', requires = IS_IN_DB(db, db.Location.id, '%(location_code)s - %(location_name)s', zero = 'Choose Location')),
        Field('mode_of_shipment','string',length = 25, requires = IS_IN_SET(['BY AIR','BY SEA','BY LAND'], zero = 'Choose Type')),
        Field('invoice_no', 'string', length = 25),
        Field('invoice_date', 'date',default = request.now.date()),
        Field('estimated_time_of_arrival', 'date', default = request.now.date()),
        Field('total_amount_in_qr','decimal(15,4)', default = 0.0),             
    ...
    if form.process().accepted:
        ...
        db.Document_Register.insert(
            ...
            estimated_time_of_arrival= form.vars.estimated_time_of_arrival,
            total_amount_in_qr= form.vars.total_amount_in_qr,            
...
def purchase_order(): # purchase_order_table
    ...
    # for n in db().select(db.Purchase_Order.ALL):
        ...
        row.append(TR(
            TD(n.purchase_order_date_approved),
            TD(n.purchase_order_no_prefix_id.prefix,n.purchase_order_no),
            TD(n.purchase_request_no_id.purchase_request_no_prefix_id.prefix,n.purchase_request_no_id.purchase_order_no),
            TD(n.dept_code_id.dept_name),
            TD(n.supplier_code_id.supp_name),
            TD(n.supplier_reference_order),
            TD(n.location_code_id.location_name),
...
def puchase_order_transaction_view_details():
    ...
    if auth.has_membership(role = 'INVENTORY SALES MANAGER') | auth.has_membership(role = 'INVENTORY'):
        ...
        head = THEAD(TR(TH('#'),TH('Item Code'),TH('Item Description'),TH('UOM'),TH('Category'),TH('Quantity'),TH('MRS Price'),TH('Total Amount'),TH('Action'),_class='bg-success'))    
...
def purchase_receipt_warehouse_grid_consolidated():
    ...
    for n in db().select(db.Purchase_Receipt_Warehouse_Consolidated.ALL, orderby = ~db.Purchase_Receipt_Warehouse_Consolidated.id):
        if n.draft == True:
            proc_lnk = A(I(_class='fas fa-highlighter'), _title='Process Row', _type='button  ', _role='button', _class='btn btn-icon-toggle', _href = URL('procurement','purchase_receipt_warehouse_grid_consolidated_processed', args = n.id, extension = False))        
...
def validate_consolidated_processed():
    row = 0    
    print request.vars['item_code'], request.vars['uom'], request.vars['pieces']
    # _len = len(request.vars['item_code'])
    for x in request.vars['item_code']:                
        if int(request.vars['uom'][row]) <= int(request.vars['pieces'][row]):
            # print 'error', request.vars['pieces'][row]
            response.js = "jQuery(errAlert())"
        else:
            row = 0
            # print 'no error', request.vars['pieces'][row]
        row += 1
...
def purchase_receipt_warehouse_grid_consolidated_processed():
    ...
    for t in db((db.Purchase_Receipt_Transaction_Consolidated.purchase_receipt_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction_Consolidated.partial == False)).select(db.Item_Master.ALL, db.Purchase_Receipt_Transaction_Consolidated.ALL, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Receipt_Transaction_Consolidated.item_code_id)):
        ...
        if t.Purchase_Receipt_Transaction_Consolidated.uom == 1:
            ...
            _pcs = INPUT(_type='number', _class='form-control pieces', _id = 'pieces', _name='pieces',_value = _pcs, _onchange = "ajax('/procurement/validate_consolidated_processed', ['item_code','uom','pieces']); return false")                       
        _qty = t.Purchase_Receipt_Transaction_Consolidated.quantity / t.Purchase_Receipt_Transaction_Consolidated.uom
        trow.append(TR(
            TD(ctr),
...
def purchase_order_reports():
    ...
    _row.append(['','','','','','Total Amount',':',_id.currency_id.mnemonic+' ' + locale.format('%.2F',_grand_total or 0, grouping = True)])
    _row.append(['','','','','','Discount %',':',locale.format('%d',_id.discount_percentage or 0, grouping = True)])
    _row.append(['','','','','','Net Amount',':', _id.currency_id.mnemonic+' ' + locale.format('%.2F',_id.total_amount_after_discount or 0, grouping = True)])
...
def warehouse_receipt_reports():
    ...
    _table = Table(_row, colWidths=[20,60,'*',30,80,55,65,90])
    _table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        # ('LINEABOVE', (0,0), (-1,0), 0.25, colors.Color(0, 0, 0, 1)),        
        ('LINEABOVE', (0,0), (-1,0), 0.25, colors.black,None, (2,2)),
...
def purchase_receipt_reports():
    _prt_rep = db(db.Purchase_Receipt.purchase_receipt_no_id_consolidated == request.args(0)).select().first()
    if not _prt_rep:
        redirect(URL('inventory','account_grid'))
    ...
    _header = [
        ...
        ['Department',':',str(_pr.dept_code_id.dept_code)+' - ' + str(_pr.dept_code_id.dept_name)]]
    _header_table = Table(_header, colWidths=['*',20,'*',20,'*',20,'*'])
    _header_table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        ('SPAN',(0,0),(-1,0)),
    ...
    _row = [['#','Item Code','Item Description','Qty','SP FC','Lnd Cost','WS-Price', 'Margin']]
    for n in db((db.Purchase_Receipt_Transaction.purchase_receipt_no_id_consolidated == request.args(0)) & (db.Purchase_Receipt_Transaction.excessed != True)).select(db.Item_Master.ALL, db.Purchase_Receipt_Transaction.ALL, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Receipt_Transaction.item_code_id)):
        ...
        _row.append([
            ...
            locale.format('%.3F',n.Purchase_Receipt_Transaction.price_cost or 0, grouping = True),
    ...
    _row.append(['','','','','','Total Amount',':', str(_pr.currency_id.mnemonic) + ' ' + locale.format('%.3F', _net_amount or 0, grouping = True)])
    _row.append(['','','','','','Discount %',':',locale.format('%.3F', _pr.discount_percentage or 0, grouping = True)])
    _row.append(['','','','','','Other Charges',':',locale.format('%.3F', _pr.other_charges or 0, grouping = True)])
    _row.append(['','','','','','Net Amount',':',  str(_pr.currency_id.mnemonic) + ' ' +locale.format('%.3F', _total_amount or 0, grouping = True)])
    _row.append(['','','','','','Net Amount (QR)',':', str('QR') + ' ' + locale.format('%.3F', _local_amount or 0, grouping = True)])
    # for m in db(db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0)).select():
    #     ctr += 1
    #     _row.append([
    #         ctr,
    ...
    #         card(m.total_pieces,m.uom)
    #     ])
    _table = Table(_row, colWidths=[20,70,'*',50,50,50,50,50])
    _table.setStyle(TableStyle([
        # ('GRID',(0,0),(-1,-1),0.5, colors.Color(0, 0, 0, 0.2)),
        # ('LINEABOVE', (0,0), (-1,0), 0.25, colors.Color(0, 0, 0, 1)),        
        ('LINEABOVE', (0,0), (-1,0), 0.25, colors.black,None, (2,2)),
    ...
    _addl = [
        ...
        [locale.format('%.3F',_pr.landed_cost or 0, grouping = True),locale.format('%.3F',_purchase_value or 0, grouping = True),_pr.currency_id.description,locale.format('%.3F',_pr.exchange_rate or 0, grouping = True),locale.format('%.3F',_pr.selective_tax or 0, grouping = True)],
        ['Remarks',_pr.remarks,'','']]
    ...
    _local_amount = float(_total_amount) * float(_pr.exchange_rate) 
    _purchase_value = float(_total_amount) * float(_pr.landed_cost)
    _row_2.append(['','','','','','Net Amount',':', str(_pr.currency_id.mnemonic) + ' ' + locale.format('%.2F', _net_amount or 0, grouping = True)])
    _row_2.append(['','','','','','Discount %',':',locale.format('%.2F', _pr.discount_percentage or 0, grouping = True)])
    _row_2.append(['','','','','','Other Charges',':',locale.format('%.2F', _pr.other_charges or 0, grouping = True)])
    _row_2.append(['','','','','','Total Amount',':',  str(_pr.currency_id.mnemonic) + ' ' +locale.format('%.2F', _total_amount or 0, grouping = True)])
    _row_2.append(['','','','','','Total Amount (QR)',':', str('QR') + ' ' + locale.format('%.2F', _local_amount or 0, grouping = True)])
    ...
    ctr_3 = 0
    _row_3 = [['#','Item Code','Item Description','Qty.','WS Price','RET Price']]
    for n in db((db.Purchase_Receipt_Transaction.purchase_receipt_no_id_consolidated == request.args(0)) & (db.Purchase_Receipt_Transaction.excessed != True)).select(db.Item_Master.ALL, db.Purchase_Receipt_Transaction.ALL, left = db.Item_Master.on(db.Item_Master.id == db.Purchase_Receipt_Transaction.item_code_id)):
        ctr_3 += 1
        _row_3.append([
            ctr_3,
    ...
    _row_4 = [['Note: Kindly check this GRV for clarity and notify Accounts of any discrepancy immediately']]
...
def document_register_report():
    ...
    _header = [
        ...
        ['Doha-Qatar'],
        ['Order No. ' + str(_list) + ' ' + str(_id.location_code_id.location_name)],
        ['Enclosed please find the following documents:-'],
        ['Suppliers Invoice No.: ' + str(_id.invoice_no) + ' dated ' + str(_id.invoice_date)],
        ['Value: QR ' + str(locale.format('%.3F',_id.total_amount_in_qr or 0, grouping = True))],
        ['Packing List'],
        ['Dispatch Note'],
