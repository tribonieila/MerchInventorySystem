...
def validate_outgoing_mail(form):
    _ip = db(db.Insurance_Proposal.id == request.vars.insurance_proposal_id).select().first()
...
def insurance_proposal_details_new():
    ...
    form = SQLFORM.factory(
        ...
        Field('insurance_proposal_id','reference Insurance_Proposal', ondelete = 'NO ACTION',requires = IS_IN_DB(db, db.Insurance_Proposal.id, '%(insurance_name)s', zero = 'Choose Insurance')),
    ...
    if form.process(onvalidation = validate_outgoing_mail).accepted:
        ...
        db.Outgoing_Mail.insert(
            ...
            insurance_proposal_id = form.vars.insurance_proposal_id,
        ...
        response.flash = 'FORM HAS ERROR'
    return dict(form = form, _ckey = _ckey, _id = _id, _om = _om)

@auth.requires_login()
def insurance_proposal_details_new_():
    _id = db(db.Insurance_Details.id == request.args(0)).select().first()
    form = SQLFORM(db.Outgoing_Mail)
    if form.process(onvalidation = validate_ins_pro).accepted:
        response.flash = 'FORM SAVE'
        # redirect(URL('procurement','insurance_proposal_reports', args = request.args(0)))
    elif form.errors:
        response.flash = 'FORM HAS ERROR'
...
def puchase_receipt_account_grid(): # manoj
    ...
    for n in db(db.Purchase_Receipt_Ordered_Consolidated.status_id == 18).select(orderby = ~db.Purchase_Receipt_Ordered_Consolidated.id):
...
def purchase_receipt_account_grid_view():
    ...
    for o in db(db.Purchase_Order.purchase_receipt_no_id == request.args(0)).select():
        ctr += 1
        _id = db(db.Purchase_Request.id == o.purchase_order_no_id).select().first()
...
def purchase_receipt_account_grid_view_transaction(): # .load
    ...
    for z in db(db.Purchase_Receipt_Transaction_Consolidated_New_Item.purchase_receipt_no_id == request.args(0)).select():
        ...
        row.append(TR(TD(ctr),TD(z.item_code),TD(z.item_description),TD(z.uom),TD(z.category_id.mnemonic),TD('0 - 0/0'),TD(_qty),TD(I(_class='fas fa-exclamation-triangle'),' NEED TO UPDATE STOCK FILES', _colspan = '3'),_class='text-danger'))          
    ...
    row.append(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(),TD(INPUT(_type='submit', _value='save',_class='btn btn-success'),TD(INPUT(_type='button', _value='abort', _class='btn btn-danger'),TD()))))
    body = TBODY(*row)    
    ...
    if form.accepts(request, session):
        ...
        for x in xrange(ctr):          
            ...
            db.Purchase_Receipt_Transaction.update_or_insert(
                ...
                purchase_ordered_quantity = request.vars['_cquantity'][x],
                price_cost = float(request.vars['price_cost'][x].replace(',','')),
                consolidated = True, received = True)
...
def validate_Purchase_Request_Transaction_Ordered(form2):
    ...
    _ex = db((db.Purchase_Receipt_Transaction.purchase_order_no_id == request.args(0)) & (db.Purchase_Receipt_Transaction.item_code_id == _id.id)).select().first()    
...
def purchase_request_form():
    ...
    if form.process().accepted:
        ...
        # print 'discount', _grand_total, _discount, _after_discount
        _id.update_record(
        ...
        redirect(URL('procurement','purchase_request_transaction'))
    elif form.errors:
...
def purchase_request():
    ...
    for n in db((db.Purchase_Request.created_by == auth.user.id) & (db.Purchase_Request.archives == False)).select(orderby = ~db.Purchase_Request.id):
        ...
        # _id = db((db.Supplier_Payment_Mode_Details.trade_terms == 1) & (db.Supplier_Payment_Mode_Details.supplier_id == n.Purchase_Request.supplier_code_id)).select().first()
        # print 'supplier_id ', _id.trade_terms
    ...
    for n in _query:
        ...
        if auth.has_membership(role = 'INVENTORY SALES MANAGER') | auth.has_membership(role = 'INVENTORY'):
            ...
            row.append(TR(
                ...
                TD(btn_lnk)))
            body = TBODY(*row)        
            foot = TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('TOTAL AMOUNT'), _align = 'right'),TD(H4(locale.format('%.2F',_id.total_amount_after_discount or 0, grouping = True)), _align = 'right'),TD()))
            foot += TFOOT(TR(TD(),TD(),TD(),TD(),TD(),TD(),TD(H4('DISCOUNT %'), _align = 'right'),TD(H4(locale.format('%.2F',_id.discount_percentage or 0, grouping = True), _align = 'right')),TD(P(_id='error'))))
...
# def validate_sales_manager(form):

# def sales_manager_approved():
#     form = SQLFORM(db.Purchase_Request, request.args(0))
#     if form.process(request, onvalidation = validate_sales_manager):
#         print 'sales manager approved'
#     return locals()
...
def purchase_history():
    ...
    for n in db().select(db.Purchase_Receipt_Ordered_Consolidated.ALL, orderby = ~db.Purchase_Receipt_Ordered_Consolidated.id):
        ctr += 1
        _grand_total += n.id
    ...
    row.append(TR(TD(),TD(),TD(),TD(B('GRAND TOTAL:')),TD(B(locale.format('%.2F',_grand_total or 0, grouping = True))),_class='active'))
    body = TBODY(*row)
    ...
    for y in db().select(db.Purchase_Request.ALL):
        tctr += 1
        tgrand_total += y.total_amount_after_discount
        trow.append(TR(TD(tctr),TD(y.purchase_request_date),TD(str(y.purchase_order_no_prefix_id.prefix), str(y.purchase_order_no)),TD(locale.format('%.2F',y.total_amount_after_discount or 0, grouping = True))))
    trow.append(TR(TD(),TD(),TD(B('GRAND TOTAL:')),TD(B(locale.format('%.2F',tgrand_total or 0, grouping = True))),_class='active'))
    tbody = TBODY(*trow)
    ttable = TABLE(*[thead, tbody], _class='table')
    _on_balanced = _sm.purchase_budget - _grand_total
...
def selected_po():
    ...
    _supplier.append(int(_pr.supplier_code_id))    
...
def insurance_proposal_reports():
    _id = db(db.Insurance_Details.purchase_request_no_id == request.args(0)).select().first()
    _ip = db(db.Insurance_Proposal.id == _id.insurance_proposal_id).select().first()
    _pr = db(db.Purchase_Request.id == request.args(0)).select().first()
    _om = db(db.Outgoing_Mail.purchase_request_no_id == request.args(0)).select().first()
    _header = [
        ['INSURANCE PROPOSAL'],
        ...
        ['Subject: ' + str(_om.mail_subject) + str(' order #') + str(_pr.purchase_order_no_prefix_id.prefix)+str(_pr.purchase_order_no),''],        
        ...
        ['VALUE',':',str(locale.format('%.2F',_pr.total_amount_after_discount or 0, grouping = True))],
        ['PAYMENT TERMS',':',str(_id.payment_terms)],
        ['MODE OF SHIPMENT',':',str(_pr.mode_of_shipment)],
        ...
        ['Expected date of arrival: ' + str(_pr.estimated_time_of_arrival.strftime("%B %d, %Y")),''],
        ['Thanking You,',''],
